...
class BackpackExchangeAdapter(ExchangeInterface):
    ...
    def get_historical_data(self, market_pair, exchange, time_unit, start_date=None, max_periods=240):
        if 'bpx' not in exchange:
            return super().get_historical_data(market_pair, exchange, time_unit, start_date, max_periods)
        ...
        historical_data = KLines(market_pair, time_unit, start_date, end_time)
        ...
        for d in historical_data:
            ...
            new_historical_data.append([int(d['end']), float(d['open']), float(d['high']), float(d['low']), float(d['close']), float(d['volume'])])
        # Sort by timestamp in ascending order
    ...
    def get_exchange_markets(self, exchanges=[], markets=[]):
        ...
        if 'bpx' not in exchanges:
            return super().get_exchange_markets(exchanges, markets)

        exchange_markets = dict()
        # convert markets item SOL/USDC to SOL_USDC
        markets = [m.replace('/', '_') for m in markets]
        for exchange in exchanges:
            all_markets = Markets()
            new_markets = dict()
            for market in all_markets:
                if market['symbol'] in markets:
                    new_markets[market['symbol']] = {
                        "precision": {
            ...
            exchange_markets[exchange] = new_markets
